...
5. **Coder**: request implementation according to the plan/spec, design (if available) and repo conventions.
6. **Tester**: write and run verification tests for the implemented changes.
7. **Reviewer**: run code review on all changes against the security/architecture + devil's advocate checklist + against the plan and design decisions.
